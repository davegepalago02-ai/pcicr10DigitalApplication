...
## 1. Overview
The PCIC Insurance Application is a **Single-Page Application (SPA)** contained entirely within one HTML file (`html_powerInsuranceApplication.html`). It uses **IndexedDB** for local storage, making it offline-capable. There is no backend server required.

## 2. Deployment Instructions

### 2.1 Distribution
To deploy the application to agents, simply distribute a folder containing:
1.  `html_powerInsuranceApplication.html` (The Application)
2.  `FarmersProfile.csv` (The Database Source)
3.  `Insurance Record.csv` (History Source)
4.  `OFFICIAL_USER_MANUAL.md` (or PDF converted version)

**Recommended Folder Structure:**
```
/PCIC_App_v1.0/
├── html_powerInsuranceApplication.html
├── DATA/
│   ├── FarmersProfile.csv
...
**Procedure:**
1. Open `html_powerInsuranceApplication.html` in a code editor.
...
3. Add a new entry to the `defaults` array:
   ...
   ```
4. Save and redistribute the HTML file.
